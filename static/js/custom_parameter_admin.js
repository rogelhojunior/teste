    ...
    window.SelectFilter = {
        init: function(field_id, field_name, is_stacked) {
            ...
            add_link.addEventListener('click', function(e) {
                ...
                if (field_id == 'id_products'){
                    SelectFilter.on_change_products()
                } else if (field_id == 'id_corbans'){
                    SelectFilter.on_change_corbans()
                }
            });
        ...
        on_change_products: function(){
            let selectedElemId = '#id_products_to'
            let selectedElem = document.querySelector(selectedElemId)
            let options = []
            $.each(selectedElem.options, function(index, option) {
                options.push(Number(option.value));
            });
            $.ajax({
                url: '/get-entities/',
                type: 'POST',
                ...
                processData: false,
                dataType: 'json',
                data: JSON.stringify({'products': options}),
                success: function(data) {
                    let entitiesField = document.querySelector('#id_entities_from');
                    entitiesField.innerHTML = '';
                    $.each(data.entities, function(index, entity) {
                        let option = new Option(entity.name, entity.id, false, false)
                        entitiesField.add(option);
                    });
                    SelectBox.init('id_entities_from');
                    SelectBox.init('id_entities_to');
                    SelectBox.move('id_entities_from', 'id_entities_to');
                    SelectFilter.refresh_icons('id_entities');
                },
        ...
        on_change_corbans: function(){
            let selectedElemId = '#id_corbans_to'
            let selectedElem = document.querySelector(selectedElemId)
            let options = []
            $.each(selectedElem.options, function(index, option) {
                options.push(Number(option.value));
            ...
            $.ajax({
                ...
                processData: false,
                dataType: 'json',
                data: JSON.stringify({'corbans': options}),
                success: function(data) {
                    let storesField = document.querySelector('#id_stores_from');
                    storesField.innerHTML = '';
                    $.each(data.stores, function(index, store) {
                        let display_text = store.id.toString() + ' - ' + store.name.toUpperCase();
                        let option = new Option(display_text, store.id, false, false)
                        storesField.add(option);
                    });
                    SelectBox.init('id_stores_from');
                    SelectBox.init('id_stores_to');
                    SelectBox.move('id_stores_from', 'id_stores_to');
                    SelectFilter.refresh_icons('id_stores');
                },
                error: function(error) {
                    console.error('Erro na requisição:', error);
                }
